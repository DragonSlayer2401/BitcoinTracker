...
`utils/benchmarkChart.utils.js` aggregates actual per-second readings into one-minute open, high, low, and close values. `utils/chartIndicators.utils.js` combines those observed candles into the selected interval while preserving sample counts and the `(interval start, interval end]` convention. Missing readings are not filled, partial candles are labeled, and line gaps remain visible. Index freshness is independent of Coinbase connection status.

Chart indicators use only complete, consecutive candles: SMA-seeded EMA 9/21, MACD 12/26 with a nine-period EMA signal and histogram, and Wilder RSI 14 with 30/70 guide levels. A gap, malformed candle, or partial/forming candle resets initialization; unknown values remain `null`. Calculations use all available loaded candles before the visible-range crop and zoom. Periods count candles: EMA 9/21 first appear after 9/21 complete candles, MACD after 26, its signal/histogram after 34, and RSI after 15 closes. For example, 15-minute MACD requires 6.5 hours and its signal requires 8.5 hours, beyond the current four-hour chart history. These visual indicators are separate from forecast inputs, probabilities, learning features, and settlement.
...
Both views support wheel/pinch zoom, dragging to pan, and keyboard-accessible zoom, pan, and reset buttons directly beneath the plot. These controls remain accessible without opening Tools, including when the visible range has no observations. A zoomed window keeps its absolute timestamps as live readings arrive and retains at least one minute. Changing history duration resets zoom; changing line/candle view preserves it. The price scale follows visible observations, and the inspection slider maps only visible points back to their original chart indexes. Zoom operates on already-loaded readings and makes no additional requests.

The chart header's Tools button opens a Chart tools modal containing `components/ChartToolbar.jsx` for view, candle interval, history, and indicator visibility, plus comparison and drawing placement controls. The price, MACD, and RSI panels share a time crosshair and zoom range. MACD and RSI have distinct backgrounds, dividers, headings, and their own value readouts in `components/ChartIndicatorPanels.jsx`. `components/ChartReadout.jsx` keeps inspected prices, OHLC, optional EMA values, and initialization notices above the plot instead of covering candles with a floating tooltip. In line view, indicator inspection uses the latest candle whose close time is at or before the inspected reading, with that close timestamp displayed; it cannot borrow a later candle close. Pin comparison in Tools saves the inspected reading or candle close for dollar and percentage comparisons; changing view or candle interval clears the pin. Expand chart opens the same chart state in a full-screen modal, where Tools, Drawings, and zoom controls remain available, and Restore returns to the dashboard.

`hooks/useChartDrawings.js` and `utils/chartDrawings.utils.js` own up to 50 horizontal lines, vertical lines, and two-point trend lines. `components/ChartDrawingControls.jsx` exposes placement controls in Tools and a separate Drawings manager from the chart header. Choosing a placement tool closes Tools so the user can draw on the chart. Drawings lists saved lines with individual deletion, Clear all drawings, undo, and keyboard-accessible exact editing of prices, local dates/times, labels, and colors. Drawings persist across events and reloads in this browser's local storage under `bitcoin-tracker:chart-drawings:cf-brti:v1`; they are not research database records. Undo in Drawings restores recent edits, additions, deletions, or clearing within the current tab session. Reloading or receiving changes from another tab resets undo history. Storage failures display a warning and preserve existing drawings. Drawing mode pauses pan/zoom; Escape cancels placement. None of these chart controls changes a forecast or places a trade.
